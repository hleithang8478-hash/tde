# -*- coding: utf-8 -*-
"""Ptrade 内运行：由 fill_config 生成；勿直接当服务运行。

禁止 import sys / import os 时，用 ctypes 切换工作目录（Windows / 无 windll / Linux）。
若工程不在 C:\\软件\\trader，请只改 PROJECT_ROOT 一行，或重新运行 fill_config.ps1。
"""
...
def _chdir_project_root(root):
    ...
    if not root:
        raise RuntimeError("PROJECT_ROOT 为空。")
    ...
    try:
        ...
        raise RuntimeError(
            "无法切换目录：无 windll/WinDLL，且 libc 不可用: %s path=%s" % (e, root)
        )

...
foreach ($f in $criticalFiles) {
    ...
    if (Test-Path -LiteralPath $full) { Pass "文件存在: $f" } else { Fail "缺少文件: $f" }
}
...
if ($null -ne $runnerSvc) {
    Warn "检测到 EMS_RUNNER 服务存在（Ptrade 场景不推荐）"
} else {
    Pass "未检测到 EMS_RUNNER 服务（符合 Ptrade 内桥接运行模式）"
}
...
try {
    ...
    if ($tnc.TcpTestSucceeded) {
        Pass "MySQL 端口连通: ${MySqlHost}:${MySqlPort}"
    } else {
        Fail "MySQL 端口不通: ${MySqlHost}:${MySqlPort}"
    }
}
catch {
...
if (Test-Path -LiteralPath $configPath) {
    ...
    if ($cfg -match "change_me_to_a_strong_token") { Fail "API token 仍是默认占位值" } else { Pass "API token 看起来已修改" }
    if ($cfg -match "change_me_to_a_very_strong_secret") { Fail "HMAC secret 仍是默认占位值" } else { Pass "HMAC secret 看起来已修改" }
    if ($cfg -match "your_user" -or $cfg -match "your_password" -or $cfg -match "your_db") {
        Fail "DB_CONFIG 仍含占位符"
    } else {
        Pass "DB_CONFIG 看起来已填写"
    }
